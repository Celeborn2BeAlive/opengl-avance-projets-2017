...
int Application::run()
{
    ...
    for (auto iterationCount = 0u; !m_GLFWHandle.shouldClose(); ++iterationCount)
    {
		...
		glUniform1i(m_uGNormalSSAOSamplerLocation, 1);
		glUniform1i(m_uTexNoiseLocation, 2);
		...
		for (GLuint i = 0; i < 64; ++i) {
			glUniform3fv(m_uSSAOSamples[i], 1, &m_ssaoKernel[i][0]);
		}
		...
		glUniform1i(m_uSSAOLocation, 0);
		glUniform1i(m_uGAmbientLocation, 1);
		glUniform1i(m_uBlurLocation, true);
        ...
        if (m_CurrentlyDisplayed == GBufferTextureCount) // Beauty
        {
           ...
           //     glBindTexture(GL_TEXTURE_2D, m_SSAOBlurColorBuffer);
               // glBindTexture(GL_TEXTURE_2D, m_SSAOColorBuffer);
               // glUniform1i(m_uShadingSSAOLocation, 5);
...

{
   ...
   // glTexImage2D(GL_TEXTURE_2D, 0, GL_RED, m_nWindowWidth, m_nWindowHeight, 0, GL_RGB, GL_FLOAT, NULL);
    glTexStorage2D(GL_TEXTURE_2D, 1, GL_R16, m_nWindowWidth, m_nWindowHeight);
...
void Application::initShadersData()
{
	...
	m_uShadingSSAOLocation = glGetUniformLocation(m_shadingPassProgram.glId(), "uSSAO");
	...
	m_SSAOProgram = glmlv::compileProgram({ m_ShadersRootPath / m_AppName / "screen.vs.glsl", m_ShadersRootPath / m_AppName / "SSAO.fs.glsl" });
	m_uGPositionSSAOSamplerLocation = glGetUniformLocation(m_SSAOProgram.glId(), "uGPosition");
	m_uGNormalSSAOSamplerLocation = glGetUniformLocation(m_SSAOProgram.glId(), "uGNormal");
	m_uTexNoiseLocation = glGetUniformLocation(m_SSAOProgram.glId(), "uTexNoise");
	m_uProjMatrixLocation = glGetUniformLocation(m_SSAOProgram.glId(), "uProjMatrix");
	...
	m_SSAOBlurProgram = glmlv::compileProgram({ m_ShadersRootPath / m_AppName / "screen.vs.glsl", m_ShadersRootPath / m_AppName / "SSAOBlur.fs.glsl" });
	m_uSSAOLocation = glGetUniformLocation(m_SSAOBlurProgram.glId(), "uSSAO");
	m_uGAmbientLocation = glGetUniformLocation(m_SSAOBlurProgram.glId(), "uGAmbient");
	m_uBlurLocation = glGetUniformLocation(m_SSAOBlurProgram.glId(), "uBlur");
}
